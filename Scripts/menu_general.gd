...
func on_ready():
	...
	if Globals.gameState_level : l_disable_buttons = ["btn_entity_editor", "btn_Quit Game"]
	disable_buttons()
...
func _on_btn_quit_to_main_menu_pressed(block_buttons_time : float = 1.0) -> void:
	...
	if is_instance_valid(Globals.World) : Globals.World._on_button_4_pressed()
...
func _on_btn_entity_editor_pressed() -> void:
	Globals.change_main_scene(load("res://Levels/debug.tscn"))

func _on_btn_example_level_pressed() -> void:
	Globals.change_main_scene(load("res://Levels/DEBUG_1.tscn"))

func _on_btn_entity_randomizer_pressed() -> void:
	Globals.change_main_scene(load("res://Levels/entity_showcase_single.tscn"))

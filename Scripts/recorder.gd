...
func _ready() -> void:
	#SaveData.delete_file("deleteme.txt", dir, dirpath)
...
func _process(delta):
	...
	for x in range(speed * 3):
		...
		if playback_index >= input_log.size():
			...
			SaveData.delete_file("input_log" + str(selected_playback_id) + ".json", dirpath)
			Globals.set_mouse_mode(true)
			Globals.message("playback finished. deleting the replay file")
		...
		var time : float = 0.0
		if "time" in event_data : time = event_data["time"]
		elif "timestamp" in event_data : time = event_data["timestamp"]
		
		if "player_posX" in event_data:
			if event_data["player_posX"] == 0:
				if not Globals.gameState_level:
		...
		if playback_timer >= time:
			...
			elif "player_posX" in event_data:
				if is_instance_valid(Globals.Player):
					Globals.Player.position = Vector2(event_data["player_posX"], event_data["player_posY"])
					Globals.Player.velocity = Vector2(0, 0)
			...
			if "Key" in event:
				...
				#print("MB")
				if "button_position_x" in event_data : simulate_mouse_click(Vector2(event_data["button_position_x"], event_data["button_position_y"]), event_data["button_index"])
				...
				if "button_position_x" in event_data : Input.warp_mouse(Vector2(event_data["button_position_x"], event_data["button_position_y"]))
...
func _on_playback_id_pressed() -> void:
	SaveData.delete_file("input_log" + str(selected_playback_id) + ".json", dirpath)
	print("input_log" + str(selected_playback_id), dir)

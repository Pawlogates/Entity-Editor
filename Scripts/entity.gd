...
@export var sprite_filepath : String = "none"
...
func movement_move_around_startPosition_y_if_not_spotted(delta):
	if not patrolling_target_spotted_active:
		movement_move_around_startPosition_y(delta)

func movement_move_around_startPosition_xy_if_not_spotted(delta):
	if not patrolling_target_spotted_active:
		movement_move_around_startPosition_x(delta)
...
func handle_reset_puzzle():
	...
	if len(reset_puzzle_nodes_inside_zone):
		if is_instance_valid(Globals.Player.camera):
